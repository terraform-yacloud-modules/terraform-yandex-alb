...
module "managed" {
  ...
  managed = {
    test-apatsev-org-ru = {
      domains         = ["test.apatsev.org.ru"]
      challenge_type  = "DNS_CNAME"
      challenge_count = 1
    }
  }

  depends_on = [module.dns_recordset]
}
...
resource "null_resource" "wait_for_certificate" {
  triggers = {
    certificate_id = module.managed.managed_certificates["test-apatsev-org-ru"].id
  }
...
module "alb" {
  ...
  listeners = {
    https = {
      ...
      cert = {
        type   = "existing"
        ids    = [module.managed.managed_certificates["test-apatsev-org-ru"].id]
        domain = "domain.com"
      }
      backend = {
        name   = "app"
